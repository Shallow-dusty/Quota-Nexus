...
.\probes\import-request-headers.ps1 -Provider opencode_go `
  -CredentialsPath .\probes\credentials.local.opencode-go-2.json
```
...
请求不含 Referer 时可通过 `-WorkspaceId` 指定。Phase 0 验证多个账号时，为每份 Credential
指定独立的 `-CredentialsPath`；探针运行时传入同一路径，避免覆盖其他账号。
