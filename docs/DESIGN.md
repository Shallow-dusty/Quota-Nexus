...
现有社区工具通常只覆盖单个供应商、单个账号或特定操作系统；另一些工具把额度查看、
账号轮换、代理路由和模型请求日志混在一起。对于只想知道“每个账号还剩多少、何时刷新、
凭据是否失效”的用户，这些方案引入了不必要的权限和运行复杂度。

本项目提供一个本地、只读、可审计的统一监控层。它不进入模型请求链路，也不接管任何
现有客户端。
...
5. 凭据只保存在操作系统安全存储中；业务数据库不保存可直接使用的秘密。
...
- 修改 OpenCode、Cline、Ollama 或其他客户端的认证文件。
- 自动切换、轮换、负载均衡或选择账号。
- 提供 OpenAI-compatible 代理端点。
- 记录或分析模型请求、提示词、回复、项目路径和代码内容。
- 购买订阅、自动续费、领取邀请奖励或管理账单。
- 批量注册账号或绕过供应商限制。
- 把账号数据同步到自建服务器或第三方云端。
- 根据本地 token 量估算真实订阅额度并冒充上游数据。
...
应用在托盘运行，每 5 分钟刷新一次。某个账号的周额度达到 85% 时，只针对该额度周期发送
一次通知；达到 95% 时再次通知。额度窗口重置后，新周期可以重新通知。
...
供应商适配器只允许访问额度查询所必需的 GET 或等价只读请求。代码中不提供以下能力：

- 写入其他应用的 `auth.json`、数据库或配置文件。
- 调用订阅、奖励、邀请、账号切换或模型推理接口。
- 代替用户自动登录网页。

请求方法和允许访问的主机、路径必须在适配器中使用显式 allowlist。重定向离开允许域名时
立即停止，不携带 Cookie 或 API Key。

### 4.2 本地攻击边界

本项目防护目标包括：

- 避免秘密出现在普通 JSON、SQLite、日志、崩溃报告和 UI 调试输出中。
- 降低其他普通用户账户直接读取凭据的风险。
...
应用仅用于监控用户有权使用的账号和工作区。它不提供账号来源、注册、共享或规避供应商
条款相关功能。
...
- 初版采用用户手动粘贴完整 `Cookie` header。
- 自动读取浏览器 Cookie 需要更高权限，推迟到后续版本，并必须显式授权。
